...
for i in range(10):
    ...
    model_p = xgb.XGBRegressor(objective='reg:squarederror', n_estimators=200, max_depth=3, learning_rate=0.3)
    ...
    rmse_list_p.append(np.sqrt(mean_squared_error(y_test_p, y_pred_p)))  # Calcul du RMSE
...
print("RMSE moyen :", np.mean(rmse_list_p))  # Affichage du RMSE moyen
print("Variance expliquée moyenne :", np.mean(explained_variance_p))

# Tracer les courbes Réelles vs Prédites pour preflop
plt.figure(figsize=(10, 6))
plt.scatter(y_test_p, y_pred_p, color='blue', label='Prédictions')
plt.plot([min(y_test_p), max(y_test_p)], [min(y_test_p), max(y_test_p)], color='red', label='Réelle vs Prédite')
plt.xlabel('Réelles')
plt.ylabel('Prédites')
plt.title('Preflop Win Rate: Réelles vs Prédites')
plt.legend()
plt.grid(True)
plt.show()
...
for i in range(10):
    ...
    # Entraînement du modèle avec XGBoost
    model_f = xgb.XGBRegressor(objective='reg:squarederror', n_estimators=100, max_depth=6, learning_rate=0.1)
    ...
    rmse_list_f.append(np.sqrt(mean_squared_error(y_test_f, y_pred_f)))  # Calcul du RMSE
...
print("RMSE moyen :", np.mean(rmse_list_f))  # Affichage du RMSE moyen
print("Variance expliquée moyenne :", np.mean(explained_variance_f))

# Tracer les courbes Réelles vs Prédites pour fullboard
plt.figure(figsize=(10, 6))
plt.scatter(y_test_f, y_pred_f, color='green', label='Prédictions')
plt.plot([min(y_test_f), max(y_test_f)], [min(y_test_f), max(y_test_f)], color='red', label='Réelle vs Prédite')
plt.xlabel('Réelles')
plt.ylabel('Prédites')
plt.title('Fullboard Win Rate: Réelles vs Prédites')
plt.legend()
plt.grid(True)
plt.show()